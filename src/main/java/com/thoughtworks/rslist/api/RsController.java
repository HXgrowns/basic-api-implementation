...
public class RsController {
    ...
    public ResponseEntity<Page<RsEventEntity>> findListByPage(@RequestParam(required = false, defaultValue = "10") Integer size, @RequestParam(required = false, defaultValue = "0") Integer page) {
        return ResponseEntity.ok(rsEventService.findListByPage(size, page));
    }

    @PatchMapping(value = "/jpa/{id}")
